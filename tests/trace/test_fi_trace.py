...
def _clear_workload_axis_state(template_mod):
...
def test_fi_trace_workload_axis_dump_writes_pid_shards(tmp_path, monkeypatch):
    ...
    _clear_workload_axis_state(template_mod)
...
def test_fi_trace_workload_axis_dump_via_decorator_without_trace_dump(
    ...
    out_path = (
        tmp_path
        / "wrapper_workload_test"
        / f"wrapper_workload_test.pid_{os.getpid()}.jsonl"
    )
...
def test_fi_trace_workload_axis_autoflush_handles_sigusr1_flush_only(
    tmp_path, monkeypatch
):
    """SIGUSR1 flushes workload-axis records without terminating the process."""
    ...
    def fail_kill(*args, **kwargs):
        raise AssertionError("SIGUSR1 flush-only handler must not terminate")
    ...
    try:
        template = TraceTemplate(
            op_type="sigusr1_workload_test",
            axes={"n": Var()},
            inputs={"input": Tensor(["n"])},
            outputs={"output": Tensor(["n"], dtype_from="input")},
        )
        fi_trace_fn = template.build_fi_trace_fn("flashinfer.tests.sigusr1_workload")

        fi_trace_fn(input=torch.empty((13,)))

        assert signal.SIGUSR1 in registered_handlers
        registered_handlers[signal.SIGUSR1](signal.SIGUSR1, None)

        out_path = (
            tmp_path
            / "sigusr1_workload_test"
            / f"sigusr1_workload_test.pid_{os.getpid()}.jsonl"
        )
        assert json.loads(out_path.read_text()) == {"axes": {"n": 13}, "count": 1}
        assert flush_workload_axis_dumps() == 0
    finally:
        _clear_workload_axis_state(template_mod)
        template_mod._WORKLOAD_AXIS_SIGNAL_HANDLERS.clear()
...
def test_fi_trace_workload_axis_autoflush_rewraps_replaced_signal_handler(
    ...
    try:
        ...
        count = sum(
            json.loads(line)["count"] for line in out_path.read_text().splitlines()
        )
        assert count == 2
